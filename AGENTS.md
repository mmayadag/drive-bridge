...
- `bun tests`: run all tests, each file isolated so module mocks cannot leak into the next (do not use `bun test`, it skips the Obsidian mock preload and the isolation).
